...
updateAt :: Int -> a -> [a] -> [a]
updateAt i x xs = take i xs ++ [x] ++ drop index xs
    where
        index  = i + 1
...
substring i j s = take amount list
    where
        list = drop i s
        amount = j - i
...
palindromify s = if rev == s then s else palindromify list
    where
        list = init (tail s)
        rev = reverse s
...
safeIndex xs i =
    let
        len = length xs
        value = i < len && i >= 0
    in if value then Just (xs!!i) else Nothing
